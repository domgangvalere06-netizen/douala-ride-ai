...
class MistralAdapter:
    ...
    def _tool_definitions(self, tool_names: list[str]) -> list[dict[str, Any]]:
        definitions = {
            ...
            "plan_trip": {"type": "function", "function": {"name": "plan_trip", "description": "Plan a complete Douala Ride journey for any pickup and destination coordinates, including real route, distance, ETA, moto/car fares and budget filtering. Use this instead of separately calculating route and fare when the passenger wants a trip plan.", "parameters": {"type": "object", "properties": {"origin": {"type": "object", "properties": {"name": {"type": "string"}, "latitude": {"type": "number"}, "longitude": {"type": "number"}}, "required": ["latitude", "longitude"]}, "destination": {"type": "object", "properties": {"name": {"type": "string"}, "latitude": {"type": "number"}, "longitude": {"type": "number"}}, "required": ["latitude", "longitude"]}, "budget_xaf": {"type": "integer", "minimum": 0}, "vehicle_type": {"type": "string", "enum": ["moto", "car"]}}, "required": ["origin", "destination"]}}},
